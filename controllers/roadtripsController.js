...
module.exports = {
  findAllSearches: function(req, res) {
    db.Roadtrip.find(req.query)
      .sort({ date: -1 })
      .then(dbModel => res.json(dbModel))
      .catch(err => res.status(422).json(err));
  },
  findById: function(req, res) {
    db.Roadtrip.findById(req.params.id)
      .then(dbModel => res.json(dbModel))
      .catch(err => res.status(422).json(err));
  },
  ...
  updateSearch: function(req, res) {
    db.Roadtrip.findOneAndUpdate({ _id: req.params.id }, req.body)
      .then(dbModel => res.json(dbModel))
      .catch(err => res.status(422).json(err));
  },
  removeSearch: function(req, res) {
    db.Roadtrip.findById({ _id: req.params.id })
