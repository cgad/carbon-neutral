...
module.exports = {
  ...
  findById: function(req, res) {
    db.Flight.findById(req.params.id)
      .then(dbModel => res.json(dbModel))
      .catch(err => res.status(422).json(err));
  },
  createFlight: function(req, res) {
    ...
    var suppAirlines = ["ALLEGIANT", "DELTA", "JETBLUE", "UNITED"];
    if (!suppAirlines.includes(req.body.airline)) {
      airline = "";
    } else {
    ...
    axios
      ...
      .then(result => {
        ...
        var newFlight = {
          ...
          cars_off_road_for_year:
            result.data.equivalents.cars_off_the_road_for_a_year,
          cars_off_road_for_day:
            result.data.equivalents.cars_off_the_road_for_a_day,
          vegan_meals:
            result.data.equivalents.vegan_meals_instead_of_non_vegan_ones,
          ...
          canisters_bbq_propane:
            result.data.equivalents.canisters_of_bbq_propane,
          homes_lowered_therm_2_deg_winter:
            result.data.equivalents
              .homes_with_lowered_thermostat_2_degrees_for_a_winter,
          homes_raised_therm_3_deg_summer:
            result.data.equivalents
  ...
  removeSearch: function(req, res) {
    db.Flight.findById({ _id: req.params.id })
